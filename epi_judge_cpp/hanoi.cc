...
void hanoi_tower(int num_rings, int from, int to, int use, vector<vector<int>>* acc) {
	if (num_rings <= 0) return;
	
	hanoi_tower(num_rings - 1, from, use, to, acc);
	acc->push_back({ from, to });
	hanoi_tower(num_rings - 1, use, to, from, acc);
}

vector<vector<int>> ComputeTowerHanoi(int num_rings) {
	vector<vector<int>> result;
	hanoi_tower(num_rings, 0, 1, 2, &result);
	return result;
}
